...
PROMPT_FILE=".cardflow-harness/prompts/m1-requirement-card/04-decide.md"
...
OUTPUT_FILE="${ROUND_DIR}/06-input-decision.md"

for file in "$PROMPT_FILE" "$CODEX_FILE" "$CLAUDE_FILE" "$DEEPSEEK_FILE" "$COMPARISON_FILE" "$SCORE_FILE"; do
  if [ ! -f "$file" ]; then
