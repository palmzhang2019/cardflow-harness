...
- **Confirmed Unknowns Before Freeze**
  ...
  角色：M1 阶段已确认事项来源；其确认结论已升格为 confirmed engineering requirements。
...
- **配置侧**：`target=顧客最新情報` 时,「項目（*必須）」中追加 `発着信日時（最新）`,位于"包含自定义项目后的全列表最末"。来源：CBR-01 / CBR-02 / CBR-03。
- **查询侧**：表形式 target=2 当前查询链路涉及 `customer_contacts` + `customer_notes(max id)` + `cdr` 关联（详见第 6 章）。本卡只确认"该项目展示值需沿用既存「顧客最新情報」链路'最新'口径"（CBR-09）；具体 SQL / join 调整方案不在本卡阶段定义。
- **画面侧**：勾选后表形式レポート查看画面能展示该项目（CBR-04）。

### 4.2 集計レポート
- **影响层**：配置页（X/Y/V 选择形式）→ 集計查询 → 集計画面。
- **配置侧**：代码事实显示「項目（*必須）」仅在 type=1（表形式）显示,集計（type=2）使用 X/Y/V 下拉（详见第 6 章）。本卡确认"集計レポート 必须支持 `発着信日時（最新）`"（CBR-05）；该项目以何种 UI 形态出现在集計配置侧（X/Y/V 下拉的哪一类、是否同时支持）属于工程未知,列入第 9 章。
- **查询侧**：代码事实显示 target=2 的集計 SQL 当前未 join `cdr`（详见第 6 章 / 第 7 章）。引入该项目对集計查询链路有结构性影响,需在 M3 阶段单独评估。
- **画面侧**：集計レポート 能展示 `発着信日時（最新）`（CBR-05）。

### 4.3 CSV 输出
- **影响层**：表形式 CSV、集計 CSV 两条独立下载链路。
- **表形式 CSV**：输出 `発着信日時（最新）`,取值口径与画面一致（CBR-06、CBR-09、CBR-10）。
- **集計 CSV**：输出 `発着信日時（最新）`,取值口径与画面一致（CBR-07、CBR-09、CBR-10）。
- **CSV 标题**：CSV 标题取字段 `name`（显示名）,需保证与配置侧 `表示名` 一致。

### 4.4 api_find_report
- **影响层**：对外 API 输出。
- **输出要求**：基于已配置字段输出 CSV 形态结果,需同步包含 `発着信日時（最新）`,取值口径与画面 / CSV 一致（CBR-08、CBR-09、CBR-10）。
...
| 编号 | 输出类型 | 备注 |
|------|----------|------|
| AO-01 | 表形式レポート 配置页（add） | 「項目（*必須）」中新增项 |
| AO-02 | 表形式レポート 配置页（edit / view） | 与 add 页可见性 / 已保存状态保持一致（CBR-12） |
| AO-03 | 表形式レポート 报表查看画面 | CBR-04 |
| AO-04 | 集計レポート 配置侧 | CBR-05；UI 形态见 9.U-E-01 |
| AO-05 | 集計レポート 报表查看画面 | CBR-05 |
| AO-06 | 表形式 CSV | CBR-06 |
| AO-07 | 集計 CSV | CBR-07 |
| AO-08 | api_find_report | CBR-08 |
| AO-09 | `/admin/cdr/index` | Reference Only / 参考面；非主动修改对象（OOS-05）。仅作为业务侧"発着信時間"语义的视觉对照,不参与本次 5 条输出链路的实现与验收 |

---

## 6. Code Investigation Facts

> 来源：Code Investigation Report。仅作为代码事实陈列。
...
- **CIF-16**：编辑页与查看页与 add 页基于同一字段定义集合渲染——`admin_edit.thtml` 与 `admin_view.thtml` 与 `admin_add.thtml` 共享 controller 内系统字段数组（CIF-01）的输出结果,因此 add 页字段定义的扩展会同步影响 edit / view 的可见性与已保存状态展示。该一致性是 CBR-12 的代码事实依据。
  来源：`app/views/customer_contact_history_reports/admin_edit.thtml`、`app/views/customer_contact_history_reports/admin_view.thtml`、`app/controllers/customer_contact_history_reports_controller.php:112`
...
- **EC-01**：「項目（*必須）」字段由 controller 内 `tableSystemFields` 数组生成（CIF-01）。新增项目仅在该数组扩展时影响表形式配置侧；不会自动影响集計 X/Y/V 下拉（CIF-06）。
- **EC-02**：表形式 target=2 的"最新"既存口径基于 `customer_notes.max(id)`（CIF-04）,与"按 `cdr.start_date` 取最大值"不等价。已确认采用既存链路口径（CBR-09）,M3 必须严格沿用既存口径,不得改写为按 `start_date` 取最新。
- **EC-03**：集計 target=2 SQL 当前无 `cdr` join（CIF-07）。集計链路引入 `発着信日時（最新）` 必然涉及查询链路结构性扩展,影响面大于表形式追加一列。
- **EC-04**：CSV 标题取 `name`（显示名）而非 `field_name`（CIF-09）。新增字段的 `name` / `field_name` 命名一致性需在 M3 落定,避免画面与 CSV 标题分歧。
- **EC-05**：既存空值显示规则不统一（空字符串 / `N/A` / `（空白）`,CIF-12）。本卡 CBR-10 已确认无数据显示 `-`,与既存任一规则均不同；M3 必须在 5 条输出链路上独立处理 `-` 显示,不能依赖既存 fallback。
- **EC-06**：配置页与下游输出之间通过 `field_name` 映射传递（CIF-03）。新增 `発着信日時（最新）` 需要在 5 条输出链路（表形式画面、集計画面、表形式 CSV、集計 CSV、api_find_report）的 `field_name → value` 映射点逐一新增分支。
- **EC-07**：順番依赖数组追加顺序（CIF-13）。CBR-03"包含自定义项目后的全列表最后"对追加位置有方向性约束：必须在系统字段 + 自定义字段拼接完成后追加。
- **EC-08**：未发现 Excel / PDF / batch / cron / 邮件相关链路（CIF-11）,作为 Out of Scope 的事实依据。
- **EC-09**：edit 页 / view 页与 add 页共享同一字段定义来源（CIF-16）。新增项目在 add 页字段数组扩展后,自然在 edit / view 中以一致可见性 / 已保存状态呈现；M3 不得在 edit / view 处建立独立的字段定义分支,以保证 CBR-12。
...
- **OOS-01**：搜索条件调整不在本次范围内。
- **OOS-02**：Excel / PDF 输出不在本次范围内（与 CIF-11 一致）。
- **OOS-03**：batch / cron 不在本次范围内（与 CIF-11 一致）。
- **OOS-04**：邮件配信内容不在本次范围内（与 CIF-11 一致）。
- **OOS-05**：`/admin/cdr/index` 自身不作为本次主动修改对象,仅作为参考页面（Reference Only）；本次不对其页面、查询、排序、字段或行为做任何变更。
- **OOS-06**：本次不主动改权限控制本身（注：`privilege.conf` 中的 `admin_report_detail_download` 历史遗留项不在本次清理范围内,CIF-14）。
...
| AC-004 | 勾选并保存后 `CustomerContactHistoryReportField` 中存在该字段记录,且 edit / view 页可见性与已保存状态与 add 页一致 | AO-01 / AO-02 | CBR-12 / EC-06 / EC-09 |
...
| AC-Regression-004 | `/admin/cdr/index` 行为不变（仅作为参考面,不参与本次修改） | AO-09 | OOS-05 |
...
- **R-06**（来源：Frozen 风险 6 + CIF-15）：`/admin/cdr/index` 默认按 `id DESC` 排序,并不严格等同于"按 `start_date` 最新"。业务侧若以该参考页视觉顺序作为对照,可能与既存链路口径产生分歧；以 CBR-09 既存链路口径为准,CDR 参考页仅作 UI 参照,不参与本次修改（与 OOS-05 / AO-09 一致）。
...
- **H-03**：M3 必须将 5 条输出链路（AO-03、AO-05、AO-06、AO-07、AO-08）作为一组验收对象处理,逐路径核对取值口径与 `-` 显示一致性。AO-09（`/admin/cdr/index`）不在该组之内,仅作为参考面。
...
- **H-10**：Out of Scope（第 8 章）项不得在 M3 阶段被悄悄扩入；尤其 `/admin/cdr/index`（AO-09 / OOS-05）必须保持为参考面,不作为主动修改目标。
- **H-11**：M3 在处理 add / edit / view 页字段一致性（CBR-12 / EC-09 / CIF-16）时,须在 `admin_edit.thtml` 与 `admin_view.thtml` 上验证新增项目的可见性与已保存状态展示与 `admin_add.thtml` 完全一致,不得在 edit / view 处引入独立字段定义分支。
